                ...
                if (idx === 0) {
                    ...
                    outContent.innerHTML = `
                        <div style="text-align: center; padding: 50px 20px; font-family: 'Malgun Gothic', sans-serif;">
                            <h2 style="color: #4a90e2; margin-bottom: 20px;"> <한글킹제임스성경 검색기 사용법> </h2>
                            <p style="font-size: 1.2em; line-height: 1.6;">
                                “하나님의 말씀은 매이지 아니하느니라.” (딤후 2:9)<br>
                                1. 서문
이 검색기는 말씀보존학회 사이트의 성경검색기를 보완하여 최대한 사용자 편의를 중점으로 제작하였습니다.
이 검색기를 허락해주신 하나님께 무한한 감사와 영광을 돌립니다.
이 프로그램을 통해 하나님께 더욱 풍성한 영광을 올려드리길 바랍니다.

2. 일러두기
본문은 말씀보존학회 사이트의 성경검색기 기준으로 하였습니다.
한글킹제임스성경에 있는 고딕체등의 글씨체 차이를 반영하지 못했습니다.
기본 서체는 맑은 고딕으로 설정해 두었습니다.
​
3. 기능 설명

읽기 모드
좌측의 성경 버튼과 장 버튼을 통해 원하는 장을 출력하여 읽을 수 있습니다.
이 경우 이전/다음 장 버튼이나 키보드 방향키를 통해 출력 장을 옮길 수 있습니다.
출력된 절의 숫자를 클릭하면 그 구절만 출력됩니다.
​
검색 모드 
검색창에 단어를 검색하거나 구절 주소를 검색하여 해당 구절을 출력할 수 있습니다.
단어 검색시 총 등장 구절의 횟수와 등장 횟수가 출력됩니다.
검색한 단어는 노란색으로 표시됩니다. 
​
구절 검색 시 검색되는 구절양식은 다음과 같습니다.

구절 검색 양식
창세기 1:1 (기본)
창 1:1 (약어)
창세기1:1 / 창1:1 (띄어쓰기 없음)
창세기 1:1 마태복음 1:1 (한번에 여러 구절 검색)
창 1:1-3 (연속 구절)
창 1:1,3,5 (비연속 구절)
검색되지 않는 구절양식은 다음과 같습니다.

검색 불가 양식
하나님 창세기 1:1 (단어와 구절을 동시에 검색 불가)
창 1:3,1 / 창 1:3-1 (구절 순서 역전 검색 불가)
창 1:1,1 / 창 1:1-1 (중복 구절 검색 불가)
창 1:1. / 창 1:1: 등등 (구절 양식 오류)
단어 검색과 구절 검색시 출력되는 양식을 버튼을 눌러 선택할 수 있습니다. 
원하는 양식에 맞게 출력하여 사용할 수 있습니다.
검색 시 출력된 구절의 구절 주소를 클릭하면 해당 장을 전부 출력하고 해당 절 번호를 노란색으로 표시합니다.
이 경우 읽기 모드로 넘어가며 읽기 모드 기능을 사용할 수 있습니다.
ctrl+z = 뒤로가기
ctrl+y = 앞으로가기
​
4. 주의 사항
단어 검색 시 띄어쓰기에 민감하지 않습니다.
구절 검색시 입력 양식이 잘못되면 단어 검색으로 인식하는 경우가 있습니다.
​​
5. 버그 제보 및 문의
오류나 버그 발견 시 제보해 주시면 하나님의 말씀을 대하는 엄중한 자세로 기쁘게 수정하겠습니다.이 화면은 일반적인 성경 검색으로는 나타나지 않습니다. 축복합니다! 🙏
                            </p>
                        </div>
                    `;
                } else {
